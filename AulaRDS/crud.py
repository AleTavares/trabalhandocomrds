...
def create_region(region_id, region_description):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO region (region_id, region_description) VALUES (%s, %s)",
        (region_id, region_description)
    )
    conn.commit()
    conn.close()

# Função para ler todas as regiões
def read_regions():
    conn = get_connection()
...
# Função para atualizar uma região existente
def update_region(region_id, region_description):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE region SET region_description = %s WHERE region_id = %s",
        (region_description, region_id)
    )
    conn.commit()
    conn.close()

# Função para deletar uma região
